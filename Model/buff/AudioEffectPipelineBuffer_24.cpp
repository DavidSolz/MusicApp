...
void AudioEffectPipelineBuffer_24::fromAudioBuffer(audioBuffer* buff){
    uint16_t l = 0;
    for (uint16_t i = 0,r = 0; i < buff->count;){
        // left[l] = buff->buff[i++];
        // left[l] += buff->buff[i++] << 8;
        // left[l] += (buff->buff[i] & 0x7F) << 16;
        // left[l++] += (buff->buff[i++] & 0x80) << 24;
        //
        // right[r] = buff->buff[i++];
        // right[r] += buff->buff[i++] << 8;
        // right[r] += (buff->buff[i] & 0x7F) << 16;
...
void AudioEffectPipelineBuffer_24::toAudioBuffer(audioBuffer* buff){
